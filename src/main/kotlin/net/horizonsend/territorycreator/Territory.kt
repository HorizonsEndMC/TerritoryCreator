...
internal class Territory(points: List<Point>, name: String) {
    ...
    fun draw(g: Graphics) {
        ...
        val x = points.stream().map { point: Point -> Integer.valueOf(point.x) }.mapToInt { i: Int? -> i!! }.toArray()
        val y = points.stream().map { point: Point -> Integer.valueOf(point.y) }.mapToInt { i: Int? -> i!! }.toArray()
